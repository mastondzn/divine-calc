...
function PoeSeparator() {
    return <div className="my-1 h-px w-full bg-linear-to-r from-transparent via-(--poe-border-dim) to-transparent" />;
}

function getPercentileColor(pct: number) {
        ...
        <div key={mod.id} className="group relative my-3 text-center">
            ...
            {mod.lines.map((line, idx) => {
                ...
                for (const [rIdx, roll] of rollsInLine.entries()) {
                    ...
                    if (splitIdx !== -1) {
                        const before = remainingLine.slice(0, Math.max(0, splitIdx));
                        ...
                        remainingLine = remainingLine.slice(Math.max(0, splitIdx + roll.text.length));
                    }
                }
...
function getRarityScheme(r: string) {
        ...
        <div className={`relative mx-auto w-full overflow-hidden rounded-xl transition-shadow duration-500 poe-border`}>
            <div className={`${rarity.gradient} rounded-t-[1px]`}>
                <div className="px-4 py-3 text-center">
                    <h2 className={`font-cinzel text-xl font-bold tracking-wider drop-shadow-md ${rarity.text}`}>
                        {item.name}
                    </h2>
                    {item.baseType && item.name !== item.baseType && (
                        <h3 className={`mt-0.5 font-cinzel text-base opacity-80 drop-shadow-sm ${rarity.text}`}>
                            {item.baseType}
                        </h3>
                    )}
                </div>
            ...
            <div className="space-y-3">
                {item.modifiers.some((m) => m.type === 'implicit' || m.type === 'enchant') && (
                    <>
                        <div>
                            {item.modifiers
                ...
                {item.modifiers.some((m) => m.type === 'unique') && (
                    <>
                        <div>
                            {item.modifiers
                                .filter((m) => m.type === 'unique')
                                .map((m) => (
                                    <ModifierBox
                                        mod={m}
                                        selectedRolls={selectedRolls}
                ...
                {item.modifiers.some((m) => m.type === 'prefix') && (
                    <>
                        <div>
                            {item.modifiers
